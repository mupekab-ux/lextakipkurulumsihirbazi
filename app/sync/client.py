...
from .config import SyncConfig, get_sync_config, save_sync_config, save_config_value
...
class SyncResult:
    ...
    incoming_changes: List[Dict[str, Any]] = field(default_factory=list)
...
class SyncClient:
    ...
    def _make_request(
        ...
        if not self.config.api_url:
            return False, "API URL yapılandırılmamış"
    ...
    def login(self, username: str, password: str) -> LoginResult:
        ...
        if not success:
            return LoginResult(success=False, message=str(result))
    ...
    def sync(self, changes: List[Dict[str, Any]] = None) -> SyncResult:
        ...
        if not success:
            return SyncResult(success=False, message=str(result))
...
def perform_full_sync() -> SyncResult:
    ...
    4. Outbox'ı temizle
    """
    from .outbox import get_pending_changes, mark_changes_synced
    from .merger import apply_incoming_changes
    ...
    return SyncResult(
        success=True,
        message=f"Senkronizasyon tamamlandı. Gönderilen: {result.changes_sent}, Alınan: {applied_count}",
        new_revision=result.new_revision,
        changes_sent=result.changes_sent,
