...
Basit ve temiz yapı:
- Her tablo için ayrı fiziksel tablo
...
class GlobalRevision(Base):
    """Global revision counter - her firma için tek sayaç."""
...
class Dosya(Base, BaseMixin):
    ...
    dava_acilis_tarihi = Column(String(20), nullable=True)
...
class Finans(Base, BaseMixin):
    ...
    dosya_uuid = Column(UUID(as_uuid=False), nullable=True, index=True)
...
class Gorev(Base, BaseMixin):
    ...
    aciklama = Column(Text, nullable=True)
    atanan_kullanicilar = Column(Text, nullable=True)
