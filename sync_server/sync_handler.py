...
Basit ve temiz sync mantığı:
1. İstemciden gelen değişiklikleri işle
2. İstemciye gönderilecek değişiklikleri hazırla
3. Global revision sayacını yönet
"""
...
def get_next_revision(db: Session, firm_id: str) -> int:
    """Firma için bir sonraki revision numarasını al ve artır (atomik)."""
...
# =============================================================================
# GÖNDERİLECEK DEĞİŞİKLİKLERİ AL
# =============================================================================
...
def get_outgoing_changes(
    ...
    last_sync_revision'dan sonra olan tüm değişiklikleri döndürür.
    """
    ...
    for table_name, model_class in SYNCABLE_TABLES.items():
        try:
            # Bu firmaya ait, belirtilen revision'dan sonra olan değişiklikleri al
...
def perform_sync(
    ...
    try:
        ...
        outgoing_changes = get_outgoing_changes(
            db=db,
            firm_id=firm_id,
            last_sync_revision=last_sync_revision,
            device_id=device_id
        )
        ...
        logger.info(f"Sync tamamlandı: firm={firm_id[:8]}..., gönderilen={processed}, alınan={len(outgoing_changes)}")
        ...
        try:
            log_sync(
                ...
                records_sent=0,
                records_received=0,
                last_revision=last_sync_revision,
                status='error',
                error_message=str(e)
            )
            db.commit()
        ...
        return {
            'success': False,
            'new_revision': last_sync_revision,
            'changes': [],
            'errors': [{"error": str(e)}],
