...
def to_str(val):
    """UUID veya herhangi bir değeri string'e çevir"""
    if val is None:
        return None
...
def init_firm(request: InitFirmRequest, db: Session = Depends(get_db)):
    ...
    # Firma oluştur - Fernet uyumlu key
    firm_key = base64.urlsafe_b64encode(secrets.token_bytes(32)).decode()
...
def get_device_status(device_id: str, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "needs_login": device.is_approved
    }
...
def logout(
    ...
    if x_device_id:
        db.query(RefreshToken).filter(
            RefreshToken.user_id == to_str(user.uuid),
...
def get_next_revision(db: Session, firm_id) -> int:
    """Sonraki revision numarasını al"""
    firm_id_str = to_str(firm_id)
    rev = db.query(GlobalRevision).filter(
        GlobalRevision.firm_id == firm_id_str
    ).with_for_update().first()

    if not rev:
        rev = GlobalRevision(firm_id=firm_id_str, current_revision=0)
        db.add(rev)
...
def process_incoming_change(
    ...
    """Gelen değişikliği işle. BN çakışma kontrolü yapar."""
